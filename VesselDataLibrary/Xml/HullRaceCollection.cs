        ...
        protected override void ProcessValidation()
        {
            for (int i = 0; i < this.Count; i++)
            {
                for (int j = i + 1; j < this.Count; j++)
                {
                    ...
                    {
                        this[j].ValidationCollection.AddValidation(DataStrings.ID, ValidationValue.IsError,
                                AMLResources.Properties.Resources.DuplicateIDFound);
                        this[i].ValidationCollection.AddValidation(DataStrings.ID, ValidationValue.IsError,
                              AMLResources.Properties.Resources.DuplicateIDFound);
                    }
                }
            }
