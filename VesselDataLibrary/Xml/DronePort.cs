  ...
  <beam_port x=" 102.14" y="8.35" z="258.74" damage="12" arcwidth="0.4" cycletime="6.0" range="1000"/>
         ...
         * */
         ...
         public static readonly DependencyProperty CycleTimeProperty =
             DependencyProperty.Register(DataStrings.CycleTime, typeof(double),
             typeof(DronePort), new PropertyMetadata(OnItemChanged));
         ...
         protected override void ProcessValidation()
         {
             ...
             if (Damage <= 0)
             {
                 base.ValidationCollection.AddValidation(DataStrings.Damage, ValidationValue.IsError,
                   string.Format(CultureInfo.CurrentCulture, AMLResources.Properties.Resources.GreaterThanZero, AMLResources.Properties.Resources.Damage));
             }
            
             if (CycleTime <= 0)
             {
                 base.ValidationCollection.AddValidation(DataStrings.CycleTime, ValidationValue.IsError,
                    string.Format(CultureInfo.CurrentCulture, AMLResources.Properties.Resources.GreaterThanZero, AMLResources.Properties.Resources.CycleTime));
             }
             
             if (Range <= 0)
             {
                 base.ValidationCollection.AddValidation(DataStrings.Range, ValidationValue.IsError,
                    string.Format(CultureInfo.CurrentCulture, AMLResources.Properties.Resources.GreaterThanZero, AMLResources.Properties.Resources.Range));

             }
             else if (Range < 100)
             {
                 base.ValidationCollection.AddValidation(DataStrings.Range, ValidationValue.IsWarnState,
                    AMLResources.Properties.Resources.ShortRange);
